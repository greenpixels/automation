...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		placeable_preview.global_position = WorldGrid.get_world_grid_coordinate_to_global(WorldGrid.get_global_to_world_grid_coordinate(event.global_position))
		...
		if event.button_mask == MOUSE_BUTTON_MASK_LEFT:
			...
			WorldGrid.remove_object_at(WorldGrid.get_global_to_world_grid_coordinate(event.global_position))
			return
			
func _place_object_at(coords: Vector2):
	if not placeable or not placeable.scene: return
...
func set_placeable(new_placeable: Placeable):
	if placeable == new_placeable: return
